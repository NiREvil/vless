...
<body>
  ...
  <div class="container">
    ...
    <footer class="footer">
      <p>© ${new Date().getFullYear()} Proxy IP Checker By <strong><a href="https://github.com/mehdi-hexing/CF-Workers-CheckProxyIP" target="_blank">mehdi-hexing</a></strong></p>
    </footer>
  </div>
  ...
  <script>
    ...
    function isValidProxyIPFormat(input) {
        ...
        const ipv6Regex = /^\[?([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}\]?$/; // Corrected
      ...
      function isIPAddress(input) {
        const ipv4Regex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
        const ipv6Regex = /^\[?([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}\]?$/; // Corrected
        const ipv6WithPortRegex = /^\[([0-9a-fA-F:]+)\]:\d+$/; // Corrected
