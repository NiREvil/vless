...
def bind_keys():
    ...
    if result.status_code == 200:
        try:
            ...
            logger.info(f"Successfully registered with client_id: {client_id[:20]}...")
...
try:
    ...
    for i in range(1, num_proxies + 1):
        # Generate keys for IPv4 proxy
        logger.info(f"Generating keys for {i} - 🇮🇷 ReGuard")
        private_key_ipv4, reserved_ipv4 = bind_keys()
        ...
        proxies_list.append(
            {
                ...
                "type": "wireguard",
                "ip": "172.16.0.2/32",
                "ipv6": f"2606:4700:110:{':'.join(f'{random.randint(0, 65535):04x}' for _ in range(4))}/128",
        ...
        # Generate keys for IPv6 proxy
        logger.info(f"Generating keys for {i} - 🇩🇪 ReGuard")
        private_key_ipv6, reserved_ipv6 = bind_keys()
