...
NUM_IPV6_ENTRY_ENDPOINTS = 2 # How many Entry proxies should use an IPv6 server endpoint
...
def register_key_on_CF(pub_key):
    ...
    try:
        ...
        r = requests.post(url, data=bodyString, headers=headers, timeout=25) # Increased timeout

        if r.status_code == 429:
            logger.warning(f"Rate limit hit (429). Headers: {r.headers}")
            retry_after = r.headers.get("Retry-After")
            wait_time = int(retry_after) if retry_after else 15 # Default wait if header missing
    ...
    try:
        ...
        if result and result.status_code == 200:
            try:
                ...
                logger.debug(f"Full API response for {key_type}: {json.dumps(response_data, indent=2)}")
                ...
                if not client_id:
                    ...
                    sys.exit(1)
                if not interface_v4:
                    logger.warning("Could not find 'v4' interface address in API response.")
                if not interface_v6:
                    logger.warning("Could not find 'v6' interface address in API response.")
...
try:
    ...
    if os.path.exists(CACHE_FILE_PATH):
         ...
         try:
             ...
             logger.error(f"Error deleting cache file: {e}. Please delete it manually and restart.")
    ...
    ipv6_dialer = f"{ip_v6_dialer}/128" if ip_v6_dialer else "2606:4700:110:8867:3f4a:906:1933:43c5/128"
    ip_entry = f"{ip_v4_entry}/32" if ip_v4_entry else "172.16.0.3/32" # Use a different fallback
    ipv6_entry = f"{ip_v6_entry}/128" if ip_v6_entry else "2606:4700:110:8867:3f4a:906:1933:43c5/128" # Different fallback
    ...
    try:
        ...
        header_comment = f"# Generated Clash Meta WireGuard config using WG script\n"
        ...
        with open(OUTPUT_YAML_FILENAME, "w", encoding="utf-8") as f:
            ...
            yaml.dump(config_template_dict, f, allow_unicode=True, sort_keys=False, default_flow_style=False, indent=2)
