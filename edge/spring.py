...
def arch_suffix():
    ...
    if machine.startswith("i386") or machine.startswith("i686"):
        ...
        raise ValueError("Unsupported CPU architecture. Supported architectures are: i386, i686, x86_64, amd64, armv8, arm64, aarch64, s390x")
...
def toSingBox(tag, clean_ip, detour):
    ...
    subprocess.run(["wget", "-N", "https://gitlab.com/fscarmen/warp/-/raw/main/api.sh"], check=True)
    prc = subprocess.run(["sudo", "bash", "api.sh", "-r"], capture_output=True, text=True)
    output = prc.stdout
